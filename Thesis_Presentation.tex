...
\begin{teo}[Convergência dos métodos contínuos para EDRs sem desaparecimento de retardo]
  ...
  % \begin{itemize}
  %   \item 
  % \label{chap3:teo:DDE_method_1:H1}
  %      A malha $\Delta=\left\{t_{0}, t_{1}, \ldots, t_{n}, \ldots, t_{N}=t_{f}\right\}$ inclui todos os pontos de descontinuidade principais e os pontos de descontinuidade de ordem $\leq p$ que se encontram em $\left[t_{0}, t_{f}\right]$, denotados por $\xi_{1}, \ldots, \xi_{s}\left(<t_{f}\right)$.
  %   \item 
  % \label{chap3:teo:DDE_method_1:H2}
  %  O método para EDOs \eqref{chap3:def:eq:ODE_method} é consistente de ordem $p$, satisfaz a condição de estabilidade 
  % \eqref{chap3:teo:ODE_methods_convergence:eq:stability_condition} e, para $k>1$, é reiniciado após cada ponto de descontinuidade $\xi_{i}, i=0,1, \ldots, s$, por um método de ordem $\geq p-1$.
  %   \item 
  % \label{chap3:teo:DDE_method_1:H3}
  %  O interpolante \eqref{chap3:def:eq:Interpolant_extension} é consistente de ordem uniforme $q$.
  %   \item 
  % \label{chap3:teo:DDE_method_1:H4}
  %      Para cada $n$, o intervalo $\left[t_{n-i_{n}}, t_{n+j_{n}+1}\right]$ onde a interpolação ocorre está incluído em $\left[\xi_{i}, \xi_{i+1}\right]$ para algum índice $0 \leq i \leq s$.
  % \end{itemize}



  % Então, o método de passos resultante tem ordem global discreta e ordem global uniforme $q^{\prime}=\min \{p, q+1\}$; ou seja,
  %
  % \[
  % \max _{1 \leq n \leq N}\left\|y\left(t_{n}\right)-y_{n}\right\|=O\left(h^{q^{\prime}} \right)
  % \]
  %
  % e
  %
  % \[
  % \max _{t_{0} \leq t \leq t_{f}}\|y(t)-\eta(t)\|=O\left(h^{q^{\prime}}\right)
  % \]
  %
  % onde $h=\max _{1 \leq n \leq N} h_{n}$. 

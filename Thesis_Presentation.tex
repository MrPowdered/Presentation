...
%     \begin{defi}[Equações Diferenciais Ordinárias (EDOs)]
%         \label{def:1:EDO} 
%         Um \textbf{Problema de Valor Inicial (PVI)} para \textbf{Equações Diferenciais Ordinárias (EDOs)} é dado por
%
%
%         \begin{equation}
%             \begin{cases}
...
%         \item[$\bullet$] Uma função $\gamma:[t_0 - p, t_f] \to \R^d$ é chamada de \textbf{solução} para o PI se $\gamma$ for contínua em $[t_0 - p, t_f]$, diferenciável em $[t_0, t_f]$ e satisfaz \eqref{def:eq:EDR}.
%         \item[$\bullet$] A função $\tau$ é chamada de \textbf{retardo} e assumisse que $\tau(t, y(t))\geq0$ para todo $t \in [t_0, t_f]$.
%
%     \end{itemize}
%     \end{defi}
%
...
%          quanto do estado $y$
%          \item[$\bullet$] \textbf{O retardo depende do tempo} caso a função retardo $\tau$ dependa apenas da tempo $t$.
%          \item[$\bullet$] \textbf{O retardo é constante} caso $\tau$ for constante.
%      \end{itemize}
%      
...
%
%     $\bullet$ Cada $\xi_{k, i}$ gera uma descontinuidade em $y^{(k+1)}$. Estes pontos são chamados de \textbf{descontinuidades primárias de k-ésimo nível}.
%
...
\begin{frame}{Teorema: Continuação}
    ...
    % \phantom{aa} $\bullet$ Portanto, $\vertiii{ \mathcal{C}_{n} \mathbf{x}} =  \max_{1 \leq j \leq q} \| C_n x^{(j)} \|_* \leq \max_{1 \leq j \leq q} \| x^{(j)} \|_*$  = \|x^{(j)}\|_*$  e $\mathcal{C}_{n}$ satisfaz a propriedade \eqref{chap3:teo:ODE_methods_convergence:eq:stability_condition}.
    ...
    \begin{figure}
        \begin{center}
            \begin{tikzpicture}[node distance=1cm,auto,>=latex',every node/.append style={align=center},int/.style={draw, minimum size=1cm}]
                ...
                (I) edge  [out=-80, in=-100] node[above] {$1 - \lambda$} (R);
            \end{tikzpicture}
        \end{center}
        \vspace{-0.6cm}
...
\begin{frame}{SIR para rumores}
    ...
    \begin{tikzpicture}[]
        % \node[anchor=south west] (image) at (0,0) {\includegraphics[width=9cm]{images/SIR_rumos.png}};
        %
        ...
        \node[anchor=south west, inner sep=0] (image) at (0,0) {\adjustbox{width=9cm}{\includegraphics{images/SIR_rumos.png}}};
